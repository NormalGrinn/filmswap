use crate::{
    Context, Error,
    database::{get_santa, get_userinfo_by_id},
    types::Phase,
    utilities::{self, ensure_dm, ensure_has_santa, ensure_joined},
};
use poise::CreateReply;
use rusqlite::Result;

#[poise::command(prefix_command, track_edits, slash_command)]
pub async fn receive(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if !ensure_has_santa(&ctx).await? {
        return Ok(());
    }
    if !crate::utilities::ensure_correct_phase(&ctx, vec![Phase::Watch]).await? {
        return Ok(());
    }
    ...
    match potentail_gift.submission {
        ...
        None => {
            ctx.say("Your Santa has not submitted a gift yet").await?;
        }
    }
    Ok(())
}
