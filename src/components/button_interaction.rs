...
use serenity::all::{
    ...
    CreateMessage, FullEvent, Interaction,
};
                ...
                if component_interaction.data.custom_id == "Join" {
                    ...
                    if phase != Phase::Join {
                        let message = CreateInteractionResponseFollowup::new()
                            .content("You cannot join in this phase!")
                            .ephemeral(true);
                    ...
                    match info {
                        Ok(user) => {
                            if user.has_joined {
                                let message = CreateInteractionResponseFollowup::new()
                                    .content("You are already in the event!")
                                    .ephemeral(true);
                                component_interaction
                                    .create_followup(&ctx.http, message)
                                    .await?;
                                ...
                                match res {
                                    Ok(_) => {
                                        let message = CreateInteractionResponseFollowup::new()
                                            .content("Successfully joined the event!")
                                            .ephemeral(true);
                                        component_interaction
                                            .create_followup(&ctx.http, message)
                                            .await?;
                                    ...
                                    Err(_) => {
                                        let message = CreateInteractionResponseFollowup::new()
                                            .content("Error joining the event")
                                            .ephemeral(true);
                                        component_interaction
                                            .create_followup(&ctx.http, message)
                                            .await?;
                        ...
                        Err(rusqlite::Error::QueryReturnedNoRows) => {
                            database::create_user(&component_interaction.user.name, user_id)
                                .await?;
                            let message = CreateInteractionResponseFollowup::new()
